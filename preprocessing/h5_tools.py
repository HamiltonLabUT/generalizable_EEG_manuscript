...
def load_event_file(event_file_name, subject, data_dir):
	if event_file_name=='TIMIT':
		event_file = '%s/%s/audio/%s_TIMIT_all_events.txt'%(data_dir, subject, subject)
	else:
		event_file = '%s/%s/audio/%s_MovieTrailers_events.txt'%(data_dir, subject, subject)
...
def scene_cut(textgrid_dir, wav_name, fs=128.0):
	sc_dict = dict()
	timing = dict()
	
	wav_name = wav_name.strip('.wav')
	wav_name = wav_name + '_corrected_SC.TextGrid'
	print(wav_name)
	r = open('%s/%s' %(textgrid_dir, wav_name))
	...
	scene_onset = scene_tier[:,0].astype(float)*128.0
	scene_onset = scene_onset.astype(int)
	scene_offset = scene_tier[:,1].astype(float)*128.0
	scene_offset = scene_offset.astype(int)

	name = wav_name.replace(textgrid_dir, '')
	name = name.replace('_SC.TextGrid', '.wav')

	sc_dict[name] = scene_onset
	timing[name] = scene_offset[-1] 
	full_mat = np.array([])

	matrix = []
	for idx, n in enumerate(sc_dict.values()):

		matrix_values = np.zeros((list(timing.values())[idx]),)
		matrix_values[n] = 1

		a = np.append(full_mat,matrix_values)
		matrix.append(a)
