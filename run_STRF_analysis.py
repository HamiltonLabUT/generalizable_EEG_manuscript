...
def plot_test_set_reps(data_dir, save_dir, stimulus_class, subject='MT0002', opacity=0.4):
	if subject == 'MT0002' :
		...
		nreps = 10 #or nreps=2
		stimulus_class = 'MT'
	f = h5py.File('%s/%s/%s_STRF_by_pitchenvsphnfeat_%s.hf5' %(data_dir, subject, subject, stimulus_class), 'r')
	cc_max = f['/cc_max_%s' %(stimulus_class.lower())][:]
...
def friedman_segmented_analysis(subject_list, auditory_env):
	...
	model_types = ['phnfeat', 'envelope', 'binned_pitch_only', 'binned_pitches']
