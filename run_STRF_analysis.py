...
<<<<<<< HEAD
	...
	scene_cut=False, scene_cut_gaborpc=False, audiovisual_gabor_sc=False, binned_pitch_spec_phnfeat=False, spec_binned_pitch=False, full_gabor_sc=False):
=======
...
>>>>>>> bba47480228fa83cce5d17b436b957233499fd4e
	"""
...
<<<<<<< HEAD
		with h5py.File('%s/%s/%s_STRF_by_pitch_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as fh: #binned pitches only
=======
...
<<<<<<< HEAD
		...
		with h5py.File('%s/%s/%s_STRF_by_envs_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as h: #envs model only 
			corr_envs = h['corrs_%s' %(stimulus_class.lower())][:]/cc_max
...
<<<<<<< HEAD
				corrs[s] = hf['corrs_%s' %(h5_type.lower())][:]/cc_max # Load the corrs
=======
...
<<<<<<< HEAD
def friedman_segmented_analysis(subject_list, model_type):
	if model_type == 'corrs_speech':
		correlations = corrs_speech
	if model_type == 'corrs_noise':
		correlations=corrs_noise
	if model_type == 'corrs_speech_noise':
=======
def friedman_segmented_analysis(subject_list, speech_only=True, speech_w_noise=False, noise_only=False):
	if speech_only:
		correlations = corrs_speech
	if noise_only:
		correlations=corrs_noise
...
=======
	...
	#extract just the corr values from the dataframe - will be used for post-hoc sign-rank tests
	pitch_x = data_['corrs'][np.where(data['STRF_type']=='binned_pitch_only')[0]]
	phnfeat_x = data['corrs'][np.where(data['STRF_type']=='phnfeat')[0]]
	envs_x = data['corrs'][np.where(data['STRF_type']=='envelope')[0]]
	totalmodel_x = data['corrs'][np.where(data['STRF_type']=='binned_pitches')[0]]
...
<<<<<<< HEAD
def weights_corr_coef(subject_list, data_dir, nchans = np.arange(64), fs = 128.0, ndelays = 77, nfeats=16, delay_min=0.0, delay_max=0.6): 
=======
...
<<<<<<< HEAD
	plt.figure(); plt.imshow(wts_matrix_timit[:,ch,nsubj].reshape(ndelays, nfeats).T, aspect='auto', cmap=cm.RdBu_r, vmin=-np.abs(wts_matrix_timit[:,ch,nsubj]).max(), vmax=np.abs(wts_matrix_timit[:,ch,nsubj]).max()) #input correctly?
=======
