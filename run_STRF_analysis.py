...
def strf_features(subject, stimulus_class, data_dir, full_gabor = False, full_audio_spec = False, 
	...
	if full_gabor_sc: 
		resp_dict, stim_dict = loadEEGh5(subject, stimulus_class, data_dir, eeg_epochs=True, resp_mean = True,
									 binaryfeatmat = True, binarymat=False, envelope=True, pitch=True, gabor_pc10=True, spectrogram=False, binned_pitches=False, spectrogram_scaled=False, scene_cut=True)
	...
	if binned_pitch_spec_phnfeat:
		resp_dict, stim_dict, freqs = loadEEGh5(subject, stimulus_class, data_dir, eeg_epochs=True, resp_mean = True, 
			binaryfeatmat = True, binarymat=False, envelope=False, pitch=False, gabor_pc10=False, spectrogram=True, 
			binned_pitches=True, spectrogram_scaled=False, scene_cut=False)

		strf_out = 'binned_pitch_spec_phnfeat_full'
...
def convexHull(stimulus_class, subject_list, data_dir, save_dir, save_fig=False):
	...
	for idx, s in enumerate(subject_list):
		...
		with h5py.File('%s/%s/%s_STRF_by_pitch_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as fh: #F0 only
...
def var_partition(stimulus_class, subject_list, data_dir, opacity=0.4):
	...
	for idx, s in enumerate(subject_list):
		...
		cc_max = f['cc_max_%s' %(stimulus_class.lower())][:] #load normalized corr values 

		with h5py.File('%s/%s/%s_STRF_by_pitchenvsphnfeat_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as h: #full model
			corr_full = h['corrs_%s' %(stimulus_class.lower())][:]/cc_max
			corr_full[np.isinf(corr_full)]=0
			p_val_full = h['pvals_%s' %(stimulus_class.lower())][:]  
		with h5py.File('%s/%s/%s_STRF_by_envspitch_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as h: #calculate phnfeat model var
			corr_envspitch=h['corrs_%s' %(stimulus_class.lower())][:]/cc_max
		...
		with h5py.File('%s/%s/%s_STRF_by_pitchphnfeat_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as h: #calculate envs model var:
			corr_pitchphnfeat=h['corrs_%s' %(stimulus_class.lower())][:]/cc_max
		...
		with h5py.File('%s/%s/%s_STRF_by_envsphnfeat_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as h: #calculate pitch model var
			corr_envsphnfeat=h['corrs_%s' %(stimulus_class.lower())][:]/cc_max
...
def cross_prediction_ConvexHull(stimulus_class, subject_list, test_cond, train_cond, test_feature, train_feature, 
	...
	for idx, s in enumerate(subject_list):
		f = h5py.File('%s/%s/%s_STRF_by_phnfeat_%s.hf5'%(data_dir, s, s, test_cond), 'r')
		cc_max=f['/cc_max_%s' %(test_cond.lower())][:]
	...
	for idx, s in enumerate(subject_list):
		...
		if train_feature == 'pitch':
			plot_color = '#64a7bc'
			resp_dict, stim_dict = loadEEGh5(s, stimulus_class, data_dir, eeg_epochs=True, resp_mean = True,
																 binaryfeatmat = False, binarymat=False, envelope=False, pitch=True, gabor_pc10=False, spectrogram=False, binned_pitches=False, spectrogram_scaled=False, scene_cut=False)
		if train_feature == 'pitchenvsphnfeat':
			plot_color = '#996666'
			resp_dict, stim_dict = loadEEGh5(s, stimulus_class, data_dir, eeg_epochs=True, resp_mean = True,
													 binaryfeatmat = True, binarymat=False, envelope=True, pitch=True, gabor_pc10=False, spectrogram=False, binned_pitches=False, spectrogram_scaled=False, scene_cut=False)
	...
	if stimulus_class == 'TIMIT':
		...
		plt.gca().set_xticks([-0.75, -0.5,-0.25, 0, 0.25, 0.50, 0.75, 1.0])
		plt.gca().set_yticks([-0.75, -0.5,-0.25, 0, 0.25, 0.50, 0.75, 1.0])
...
def ANOVA_stats(subject_list, data_dir, h5_type, model_types):
	...
	for model in model_types: # 3 total models we are comparing
		for s in subject_list:
			f = h5py.File('%s/%s/%s_STRF_by_phnfeat_%s.hf5'%(data_dir, s, s, h5_type), 'r')
			cc_max=f['/cc_max_%s' %(h5_type.lower())][:]
	...
	for model in model_types: # 3 total models we are comparing
		for s in subject_list:
			...
			with h5py.File(strf_file,'r') as hf:
				corrs[s] = hf['corrs_%s' %(h5_type.lower())][:]/cc_max # Load the corrs and normalize 
	...
	print('Full auditory model vs. pitch: ')
	...
	print('phonological features vs. pitch: ')
	print(pg.wilcoxon(phnfeat_x,pitch_x, tail='two-sided'))
	print('acoustic envelope vs. pitch: ')
...
def num_of_stim_reps(subject, stimulus_class, data_dir, save_dir, save_fig=True):
	...
	with h5py.File('%s/%s_%s_corrs_reps.hf5' %(data_dir, subject, stimulus_class), 'w') as g:
...
def plot_test_set_reps(data_dir, save_dir, stimulus_class, subject='MT0002', opacity=0.4):
	...
	cc_max = f['/cc_max_%s' %(stimulus_class.lower())][:] #using binned_pitches as feature because MT0020 only has cc_max in this file.
	#the normalized corrs are the same for each subject regardless of which feature/s you load
	...
	with h5py.File('%s/%s/%s_%s_corrs_reps.hf5' %(data_dir, subject, subject, stimulus_class), 'r') as fh:
	...
	if subject == 'MT0020':
		...
		#conduct wilcoxon between two and 10 repetitions for MT0020 movie trailers
		print('printing wilcoxon test between two and 10 reps')
		print(pg.wilcoxon(all_corrs[9].mean(0), all_corrs[1].mean(0), tail='two-sided'))
		...
		plt.plot([0.0, 0.25], [0.0, 0.25], 'black', label='unity')
...
def trailer_aud_segmentation(subject_list, data_dir, ts, stim_type, sfreq=128.0, delay_max=0.6, delay_min=0.0, timepoints=None):
	...
	for idx, subject in enumerate(subject_list):
		...
		stims = stims/stims.max(0) #take mean across time dimension 
...
def auditory_environment_trailers_correlation(grid_dir, subject_list, data_dir, save_dir, grid_name = 'insideout-tlr2zzyy32_a720p_corrected.TextGrid', 
	ts='insideout-tlr2zzyy32_a720p.wav', ntimes=17364, save_fig=True):
	...
	stim_type = ['phnfeat', 'envelope', 'pitch', 'pitchenvsphnfeat']
	...
	for j, z in enumerate(stim_type):
		...
		# for subject in np.arange(resp.shape[0]):
			...
			if z == 'phnfeat':
				noise_resp = np.zeros((1,noise_resp.shape[1], 64))
	...
	return corrs_speech, corrs_noise, corrs_speech_noise

def speech_nonspeech_tier_alignment_slidingCorr(grid_dir, grid_name, ntimes=17364, sfreq=128.0):
	'''
...
def friedman_segmented_analysis(subject_list, corrs_speech=False, corrs_noise=False, corrs_speech_noise=False):
	if auditory_env == 'speech_only':
		correlations = corrs_speech
	if auditory_env == 'noise_only':
		correlations=corrs_noise
	if auditory_env == 'speech_w_noise':
...
def binned10Pitch_vs_pitch(data_dir, save_dir, subject_list, stimulus_class, full_model = True, pitch_only=False, save_fig=True):
	corrs = []
	for idx, s in enumerate(subject_list):
		if full_model: #assessing pitch from full model
			with h5py.File('%s/%s/%s_STRF_by_pitchenvsphnfeat_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as fh: #full model (old pitch)
				f0_model = fh['corrs_%s_norm' %(stimulus_class.lower())][:]  
				
			with h5py.File('%s/%s/%s_STRF_by_binned_pitches_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as h: #full model, with binned pitch 
				binned_pitch = h['corrs_%s_norm' %(stimulus_class.lower())][:]
			saving = 'fullmodel_vs_fullbinnedpitch'

		if pitch_only: #assessing pitch as individual feature
		...
		corrs.append([f0_model, binned_pitch]) #first feature = old pitch, second feat. = binned pitch
	points = np.hstack(corrs).T

	plt.plot(points[:,0], points[:,1], '.') #F0 feature always on x-axis, with binned pitch on y-axis
	plt.xlabel('F0 pitch')
	...
	if save_fig:
		plt.savefig('%s/%s_%s.pdf' %(save_dir, stimulus_class, saving))


def gabor_scenecut_scatter(data_dir, save_dir, subject_list, stimulus_class='MT', gabor_v_combined_model=True, sc_v_combined_model=False, save_fig=True):
	corrs = []
	for idx, s in enumerate(subject_list):
...
def convexHull_spec(stimulus_class, subject_list, data_dir, save_dir, save_fig=True): 
	...
	for idx, s in enumerate(subject_list):
		...
		with h5py.File('%s/%s/%s_STRF_by_pitch_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as h: #pitch model only 
			pitch = h['corrs_%s' %(stimulus_class.lower())][:]/cc_max
			pitch[np.isinf(pitch)]=0
			p_vals_pitch = h['pvals_%s' %(stimulus_class.lower())][:]
			pitch_sig = (pitch[np.where(p_vals_full[0] < 0.05)])
		with h5py.File('%s/%s/%s_STRF_by_phnfeat_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as h: #phnfeat model only 
			phnfeat = h['corrs_%s' %(stimulus_class.lower())][:]/cc_max
			phnfeat[np.isinf(phnfeat)]=0
			p_vals_phnfeat = h['pvals_%s' %(stimulus_class.lower())][:]
			phnfeat_sig = (phnfeat[np.where(p_vals_full[0] < 0.05)])

		corrs.append([fullModel, spec, phnfeat, pitch])
		corrs_sig.append([full_sig, spec_sig,  phnfeat_sig, pitch_sig])

	points = np.hstack(corrs).T
	points_sig = np.hstack(corrs_sig).T

	hull1=ConvexHull(np.vstack((points_sig[:,0] , points_sig[:,1])).T) #full model, spec
	hullv1 = hull1.vertices.copy()
	hullv1 = np.append(hullv1, hullv1[0])
	print(hullv1)
...
def unique_variance_Barplot_spec(stimulus_class, subject_list, venn_diagram_spec, save_dir, opacity=0.4, save_fig=True):
	...
	for idx, s in enumerate(subject_list):
		...
		unique_var_pitch.append(venn_diagram_spec[idx][2]) #spec_U_phnfeat
		unique_var_phnfeat.append(venn_diagram_spec[idx][1]) #spec_U_pitch
		unique_var_spec.append(venn_diagram_spec[idx][0]) #pitch_U_phnfeat
...
def weights_corr_coef(subject_list, data_dir, save_dir, nchans = np.arange(64), fs = 128.0, ndelays = 77, nfeats=16, delay_min=0.0, delay_max=0.6): 
	...
	for stimulus_class in stimulus_classes:
		...
		for idx, s in enumerate(subject_list):
			with h5py.File('%s/%s/%s_STRF_by_pitchenvsphnfeat_%s.hf5'%(data_dir, s, s, stimulus_class), 'r') as fh: #full model
	...
	plt.figure(); plt.imshow(wts_matrix_mt[:,ch,nsubj].reshape(ndelays, nfeats).T, aspect='auto', cmap=cm.RdBu_r, vmin=-np.abs(wts_matrix_mt[:,ch,nsubj]).max(), vmax=np.abs(wts_matrix_mt[:,ch,nsubj]).max())
	plt.colorbar()
	...
	plt.figure(); plt.imshow(wts_matrix_timit[:,ch,nsubj].reshape(ndelays, nfeats).T, aspect='auto', cmap=cm.RdBu, vmin=-np.abs(wts_matrix_timit[:,ch,nsubj]).max(), vmax=np.abs(wts_matrix_timit[:,ch,nsubj]).max())
	plt.colorbar()
	t = np.linspace(delay_min, delay_max, ndelays)
	plt.gca().set_xticks([0, (ndelays-1)/2, ndelays-1])
	plt.gca().set_xticklabels([t[0], t[np.int((ndelays-1)/2)], t[ndelays-1]])
...
def covariance_feature_mat(stimulus_class, data_dir, save_dir, subject='MT0002'):
	...
	resp_dict, stim_dict, freqs = loadEEGh5(subject, stimulus_class, data_dir,
		...
		eeg_epochs=True, resp_mean = True, binarymat=False, binaryfeatmat = True, envelope=True, pitch=True, gabor_pc10=False, 
		spectrogram=True, binned_pitches=False, spectrogram_scaled=False, scene_cut=False)
	...
	new_feats = feat_labels + spec_labels.tolist() 
